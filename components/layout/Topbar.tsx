...
const Topbar = () => {
  ...
  const [isSheetOpen, setIsSheetOpen] = useState(false); // State để quản lý trạng thái mở/đóng của Sheet
        ...
        <div className="max-sm:hidden flex gap-6">
          {topRoutes.map((route) => (
            <Link
              href={route.path}
              key={route.path}
              className={`text-sm font-medium hover:bg-[#f1e1bf] p-1 rounded-lg ${
                route.path === "/"
                  ? pathName === route.path && "bg-[#FDAB04]"
              ...
              <div className="flex flex-col gap-4">
                {topRoutes.map((route) => (
                  <Link
                    ...
                    onClick={handleLinkClick} // Đóng Sheet khi link được click
                  >
                    {route.label}
                  </Link>
                ))}
